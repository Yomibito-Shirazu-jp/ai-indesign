...
function resolveHostBridge() {
    // One panel package supports all hosts; pick module + port at runtime.
    try {
        const { app } = require("indesign");
        return { app, appName: "InDesign", port: 49300 };
    } catch {}
    try {
        const { app } = require("illustrator");
        return { app, appName: "Illustrator", port: 49301 };
    } catch {}
    try {
        const { app } = require("photoshop");
        return { app, appName: "Photoshop", port: 49302 };
    } catch {}
    throw new Error("Unsupported host app");
}

const hostBridge = resolveHostBridge();
const app = hostBridge.app;
...
function connect() {
    ...
    ws = new WebSocket(`ws://localhost:${hostBridge.port}`);
    ...
    panels: {
        mainPanel: {
            show() {
                ...
                if (appName) appName.textContent = hostBridge.appName;
                const hintApp = document.getElementById("hint-app");
                if (hintApp) hintApp.textContent = hostBridge.appName;
                connect();
            }
        }
    }
