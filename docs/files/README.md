...
3. **InDesignの「すべての機能」の操作**
   InDesignには星の数ほど機能がありますが、現状このツールが対応しているのは179種類の操作ツールです（全一覧は [`tool_list.md`](tool_list.md) を参照）。徐々にできることは増えていきますが、マニアックな処理はAIから「その機能はまだ使えません」と返ってくることがあります。
...
### Q. 「MCP？ UXP？ よく似た横文字が出てきますが、何が違うんですか？」

**A. どちらも「つなぐためのルール（言葉）」のことですが、話しかける相手が違います。**

- **MCP（Model Context Protocol）**: **「AI」と「外部のシステム」**をつなぐための世界共通語です。Claude DesktopなどのAIが、裏側で動いているこのシステムに「〇〇をやって！」と命令するために使います。
- **UXP（Unified Extensibility Platform）**: **「外部のシステム」と「Adobeのソフト」**をつなぐための、Adobeが作った新しい共通語のことです。ソフトの中に小窓（プラグイン）を作り、実際にソフトを動かすために使います。
  ※かつてのAdobe拡張機能形式は「MXP（Managed Extension Package）」や「CEP（Common Extensibility Platform）」と呼ばれていましたが、UXPはそれらに代わる最新の仕組みです。
...
**A. いいえ、残念ながらそれでは解決しません。むしろライセンス違反になるのでお控えください。**
...
本プロジェクトでは、海外製ツールでは対応しきれない「日本のDTP特有の機能」を実現するために、以下の専用処理ファイル（ハンドラーとツール定義）を独自実装しています。機能追加や改修を行いたいエンジニアの方は、以下のファイル群をご参照ください。

| 処理本体（`src/handlers/`） | ツール定義（`src/types/`） | 搭載されている主な機能 |
| :--- | :--- | :--- |
| `japaneseTypesettingHandlers.js` | `toolDefinitionsJapanese.js` | **日本語DTP組版のコア処理**: ルビ（ふりがな）の自動振り、禁則処理（行頭に「。」が来ないようにする等）、文字ツメ、和欧文字間隔の調整 |
| `verticalTextHandlers.js` | `toolDefinitionsVertical.js` | **縦書き特化処理**: テキストフレームの縦書き化、縦中横（たてちゅうよこ／2桁数字の横組み等）、連数字、右綴じドキュメントへの自動変換 |
| `proofreadingHandlers.js` | `toolDefinitionsProofreading.js` | **日本語自動校正**: 「引越」と「引っ越し」などの表記ゆれ検知、常用漢字外のチェック、差別用語や放送禁止用語などの要注意ワードの検知 |
| `textFlowHandlers.js` | `toolDefinitionsTextFlow.js` | **長文の自動流し込み**: Word等の長文テキストを流し込む際、オーバーセット（文字溢れ）が発生したら自動で次ページを追加して連結する処理 |
| `preflightHandlers.js` | `toolDefinitionsPreflight.js` | **入稿前プリフライト**: フォントのエンベッド制限チェック、スミベタ（K100%）の強制オーバープリント化、PDF/X-1a等の安全な入稿データ作成 |

※ 各ハンドラーとツール定義は `src/index.js` で登録・紐付けされています。新しいツールの追加方法は [`CONTRIBUTING.md`](CONTRIBUTING.md) をご覧ください。
...
この素晴らしいベースシステムがあったからこそ、日本の複雑なDTP環境や縦書きの悩みを解決するための「AI in Design」というプロジェクトへと発展させることができました。本当にありがとうございます！
